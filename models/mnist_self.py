...
class MnistSelf(mnist.MNIST):
    ...
    def _load_data(self):
        image_file = f"{'train' if self.train else 't10k'}-images-idx3-ubyte{'-self' if self.train else ''}-{self.concept}"
        data = mnist.read_image_file(os.path.join(self.raw_folder, image_file))

        label_file = f"{'train' if self.train else 't10k'}-labels-idx1-ubyte{'-self' if self.train else ''}"
